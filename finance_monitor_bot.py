...
Updated to use Yahoo Finance with EUR conversion
"""
...
class FinanceMonitor:
    ...
    def parse_google_finance_price(self, html_content: str) -> Optional[Dict]:
        ...
        try:
            ...
            if price_elem and price_elem.get('data-last-price'):
                try:
                    current_price = float(price_elem['data-last-price'])
                except:
                    pass
            
            # Strategy 2: Look for specific class patterns (Google changes these frequently)
            if not current_price:
                for class_pattern in ['YMlKec fxKbKc', 'YMlKec', 'fxKbKc', 'rPF6Lc', 'lu0oZ']:
                    price_elem = soup.find('div', class_=class_pattern)
                    if price_elem:
            ...
            # Strategy 3: Look for price in meta tags
            if not current_price:
                meta_price = soup.find('meta', {'itemprop': 'price'})
                if meta_price and meta_price.get('content'):
                    try:
                        current_price = float(meta_price['content'])
                    except:
                        pass
            
            # Strategy 4: Search for price pattern in text
            if not current_price:
                # Look for patterns like "12.34" or "1,234.56" in prominent text
                for elem in soup.find_all(['span', 'div']):
                    if elem.text:
                        # Match price patterns
                        match = re.search(r'^\s*(\d{1,3}(?:[,\s]\d{3})*(?:\.\d+)?)\s*
...
, elem.text.strip())
                        if match:
                            try:
                                ...
                                # Sanity check - stock prices typically between 0.01 and 10000
                                if 0.01 <= price <= 10000:
                                    current_price = price
                                    break
                            except:
            ...
            # Find price change - multiple strategies
            change_percent = 0.0
            ...
            prev_patterns = ['Previous close', 'Prev close', 'Close']
            for pattern in prev_patterns:
                prev_elem = soup.find(text=re.compile(pattern, re.I))
                if prev_elem:
                    parent = prev_elem.parent
                    if parent:
                        # Look for price in next sibling or parent
                        for elem in [parent.find_next_sibling(), parent.parent]:
                            if elem and elem.text:
                                match = re.search(r'(\d+\.?\d*)', elem.text.strip().replace(',', ''))
                                if match:
                                    try:
                                        prev_close = float(match.group(1))
                                        break
                                    except:
                                        continue
            
            # Calculate change if we have previous close
            if change_percent == 0.0 and prev_close != current_price and prev_close > 0:
